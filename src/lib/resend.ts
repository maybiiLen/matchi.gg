...
export async function sendWelcomeEmail(email: string) {
  ...
  const { data, error } = await resend.emails.send({
    ...
    html: `
      <div style="background-color: #ffffff; color: #1a1a1a; padding: 40px; font-family: system-ui, -apple-system, sans-serif;">
        <div style="max-width: 480px; margin: 0 auto;">
          <h1 style="color: #1a1a1a; font-size: 28px; margin-bottom: 16px;">Welcome to Matchi</h1>
          ...
          <div style="margin-top: 32px; padding-top: 24px; border-top: 1px solid #e5e5e5;">
            <p style="color: #999999; font-size: 13px;">GG,<br/>The Matchi Team</p>
          </div>
        </div>
      </div>
    `,
